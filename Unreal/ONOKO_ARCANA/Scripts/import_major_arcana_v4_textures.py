...
STAGING_DIR = os.path.join(PROJECT_DIR, "ImportStaging", "MajorArcana_V4")
RESULT_PATH = os.path.join(STAGING_DIR, "v4-import-result.json")

CARD_TARGET_ROOT = "/Game/ONOKOArcana/Cards/Textures/V4"
QA_TARGET_ROOT = "/Game/ONOKOArcana/QA/Textures"

ITEMS = [
    {
        "id": "major-00-fool",
        "file": "major-00-fool-onoko-v4-alpha.png",
        "destinationPath": CARD_TARGET_ROOT,
        "destinationName": "T_Major_00_Fool_ONOKO_V4_Alpha",
    },
    {
        "id": "major-01-magician",
        "file": "major-01-magician-onoko-v4-alpha.png",
        "destinationPath": CARD_TARGET_ROOT,
        "destinationName": "T_Major_01_Magician_ONOKO_V4_Alpha",
    },
    {
        "id": "major-02-high-priestess",
        "file": "major-02-high-priestess-onoko-v4-alpha.png",
        "destinationPath": CARD_TARGET_ROOT,
        "destinationName": "T_Major_02_HighPriestess_ONOKO_V4_Alpha",
    },
    {
        "id": "card-back",
        "file": "card-back-onoko-v4-alpha.png",
        "destinationPath": CARD_TARGET_ROOT,
        "destinationName": "T_CardBack_ONOKO_V4_Alpha",
    },
    {
        "id": "qa-checkerboard",
...
def log(message):
    unreal.log("[ONOKO_ARCANA_V4_IMPORT] " + message)
    print("[ONOKO_ARCANA_V4_IMPORT] " + message)
...
def main():
    ...
    for task, item in zip(tasks, ITEMS):
        ...
        expected_size = (1024, 1536) if item["id"] != "qa-checkerboard" else (1024, 1024)
        results.append(
            {
                "id": item["id"],
    ...
    if not payload["ok"]:
        raise RuntimeError("One or more V4 textures did not match expected dimensions")
    log(f"Imported {len(results)} V4/QA textures")
...
try:
    ...
    unreal.log_error("[ONOKO_ARCANA_V4_IMPORT] " + str(exc))
    print("[ONOKO_ARCANA_V4_IMPORT_ERROR] " + str(exc), file=sys.stderr)
    raise
