...
OUTPUT_NAME = "v4-alpha-qa-map-unreal-render-2026-06-01.png"
OUTPUT_PATH = os.path.join(OUTPUT_DIR, OUTPUT_NAME)
RESULT_PATH = os.path.join(PROJECT_DIR, "Saved", "V4AlphaQARenderResult.json")


def log(message):
    unreal.log("[ONOKO_ARCANA_V4_ALPHA_RENDER] " + message)
    print("[ONOKO_ARCANA_V4_ALPHA_RENDER] " + message)
...
def create_render_target(width, height):
    ...
    for enum_name in ("TextureRenderTargetFormat", "RenderTargetFormat"):
        ...
        enum_value = getattr(enum_type, "RTF_RGBA8", None) if enum_type else None
        if enum_value and set_property_if_available(render_target, "render_target_format", enum_value):
            break
    try:
...
def main():
    ...
    render_target = create_render_target(2048, 1536)
    capture_actor = unreal.EditorLevelLibrary.spawn_actor_from_class(
        unreal.SceneCapture2D,
        unreal.Vector(0.0, 120.0, 950.0),
        unreal.Rotator(-90.0, 0.0, 0.0),
    )
    try:
        capture_actor.set_actor_label("QA_Alpha_Topdown_RenderCapture")
    except Exception:
        pass
    ...
    set_property_if_available(capture, "ortho_width", 760.0)
    ...
    result = {
        ...
        "outputSize": [2048, 1536],
        "capture": {
            "location": [0.0, 120.0, 950.0],
...
try:
    ...
    unreal.log_error("[ONOKO_ARCANA_V4_ALPHA_RENDER] " + str(exc))
    print("[ONOKO_ARCANA_V4_ALPHA_RENDER_ERROR] " + str(exc), file=sys.stderr)
    raise
